  ...
  try {
    ...
    for (let i = text.length - 1; i >= 0; i--) {
      ...
      try {
        ...
        const ruling = String(obj.ruling ?? obj.verdict ?? obj.decision ?? '').toUpperCase();
        if (obj && typeof obj === 'object' && ('ruling' in obj || 'verdict' in obj || 'decision' in obj)) {
          return {
            ruling: ruling === 'CLAIMANT' ? 'CLAIMANT_WIN' : ruling === 'RESPONDENT' ? 'RESPONDENT_WIN' : ruling === 'DISMISS' ? 'DISMISSED' : ruling,
            confidence:
